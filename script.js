...
// decide whether the website should start in "light" or "dark" mode.
    ...
    handleScrolling() {
        ...
        document.addEventListener('visibilitychange', () => {
            ...
            if (!this.isPageVisible) {
                ...
                // Resume the slider when the page becomes visible again
                if (!this.autoSlideInterval && this.elements.beforeAfterSlider && !this.isSliderPaused) {
                    this.initMultiBeforeAfterSlider(); // Re-initialize to be safe
                }
            }
        });
    }
    ...
    initMultiBeforeAfterSlider() {
        ...
        // ====================================================================
        // ===== 1. HOW TO ADD, REMOVE, OR CHANGE SLIDER IMAGES =====
        // ====================================================================
        // To add more projects, add a new object {} to this 'imageSets' array.
        // Each object must have a 'before' image path, an 'after' image path, and a 'title'.
        // The slider will automatically cycle through all the projects you add here.
        //
        // To remove a project, simply delete its corresponding object from the array.
        // To change images, just replace the path in 'before:' or 'after:'.
        ...
        const updateSlider = (position) => {
            if (isMobile) {
                ...
                position = Math.max(-5, Math.min(105, position));
        ...
        const startAutoSlide = () => {
            if (this.autoSlideInterval || isSliderPaused) return;
             ...
             this.autoSlideInterval = setInterval(() => {
                 if (isMobile && !isTransitioning) {
                    ...
                    // ====================================================================
                    // The number '1.0' below controls how many percentage points the slider moves in each 'frame'.
                    // Increase it for a FASTER slider (e.g., 2.0).
                    // Decrease it for a SLOWER slider (e.g., 0.5).
                 ...
                 // The number '80' below is the interval in milliseconds (ms) for each 'frame'.
                 // Decrease it for a FASTER, smoother slider (e.g., 40).
                 // Increase it for a SLOWER, choppier slider (e.g., 120).
        ...
        if (isMobile) {
         ...
         if (isMobile) {
             ...
             updateImageScroll(0);
                         ...
                         // ====================================================================
                         // ===== 3. HOW TO CHANGE THE SLIDER'S INITIAL START DELAY =====
                         // ====================================================================
                         // The number '1000' below is the delay in milliseconds (ms) before the slider starts moving.
                         // 1000ms = 1 second.
                         // Increase it for a longer delay (e.g., 3000 for 3 seconds).
                         // Decrease it for a shorter delay (e.g., 500 for half a second).
        ...
        let skillObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                ...
                if (entry.isIntersecting) {
                    ...
                    const skills = $(section).find('.skill');
                    const numItems = skills.length;
                    const totalDuration = 5000; // 5 seconds total
                    const itemDuration = totalDuration / numItems;
                    const delayBetweenItems = itemDuration * 0.5;
                    const allSkillsAnimationFinishedTime = ((numItems - 1) * delayBetweenItems) + itemDuration;

                    // Set a timeout to remove transition-delay after the animation
                    setTimeout(() => {
                        section.classList.add('animation-finished');
                    }, allSkillsAnimationFinishedTime);

                    // Start dot animations 2 seconds after all skills are done
                    setTimeout(() => {
                        if (section.classList.contains('in-view')) { // Check if still in view
                            $(section).find('.moving-dot').css('animation-play-state', 'running');
                        }
                    }, allSkillsAnimationFinishedTime + 1000);

                } else {
                    // Reset animation when out of view
                    section.classList.remove('in-view', 'animation-finished', 'skill-is-active');
                    $(section).find('.skill-info').removeClass('can-scroll-down');
                    
                    // Pause all dot animations immediately
                    $(section).find('.moving-dot').css('animation-play-state', 'paused');
                    ...
                    // Reset the circle layout and transitions
                    ...
                    skills.each(function(index) {
                        const skillAnimationDelay = index * delayBetweenItems;
                        $(this).css({
                            'transition': `all ${itemDuration}ms ease ${skillAnimationDelay}ms`
                        });
                    });
                }
            });
...
// ===== PORTFOLIO SKILLS FUNCTIONALITY =====
        ...
        function createCircle(containerId, skills) {
            ...
            const totalDuration = 5000; // 5 seconds total
            const itemDuration = totalDuration / numItems;
            const delayBetweenItems = itemDuration * 0.5; // Start next item when previous is 50% done
            
            // Create skills and position them in a circle
            skills.forEach((skill, index) => {
                const angle = (index * (2 * Math.PI / numItems)) - (Math.PI / 2); // Start from top
                ...
                // Set initial position and new animation properties
                skillElement.css({
                    left: x + 'px',
                    top: y + 'px',
                    'transition': `all ${itemDuration}ms ease ${skillAnimationDelay}ms`
                ...
                // Create and append the dot with a delay
                ...
                skillElement.find('.dot-container').append(dot);
 ...
 $(document).ready(function() {
        ...
        $("body").on("click", ".skill", function() {
            ...
            if ($(this).hasClass('skill-active')) {
                ...
                $skillInfo.removeClass('can-scroll-down'); // Hide scroll shadow
                resetToCircle($container);
                return;
            }
